...
export interface TesterPromptVariables {
    workingDir: string;
}


export const TESTER_PROMPT = ({ workingDir }: TesterPromptVariables) => `
...
1.  **Analyze Implementation:** Review the code files created or modified by the Coder (typically in ${workingDir}). Use tools to read these files.
...
2.  **Understand:** Use \`${ToolName.GLOB}\` and \`${ToolName.GREP}\` search tools extensively to understand file structures, existing code patterns, and conventions.
3.  **Investigate:**
...
5.  **Run & Capture Results:**
    *   **ALWAYS RUN THE CODE AND TESTS** - Use \`${ToolName.RUN_SHELL}\` to execute the test suite (e.g., \`npm test\`, \`jest --coverage\`), always explaining each command.
...
7.  **Verify Standards:** Run project-specific build, linting and type-checking commands (e.g., 'tsc', 'npm run lint', 'ruff check .') to ensure code quality and adherence to standards.
8.  **Report:**
