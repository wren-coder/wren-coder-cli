...
import { getErrorMessage } from '@wren/wren-coder-core';
...
describe('memoryCommand', () => {
  ...
  describe('/memory show', () => {
    ...
    let mockGetMdFileCount: Mock;

    beforeEach(() => {
      showCommand = getSubCommand('show');

      mockGetUserMemory = vi.fn();
      mockGetMdFileCount = vi.fn();

      mockContext = createMockCommandContext({
        services: {
          config: {
            getUserMemory: mockGetUserMemory,
            getMdFileCount: mockGetMdFileCount,
          },
        },
      });
    });
    ...
    it('should display a message if memory is empty', async () => {
      ...
      mockGetMdFileCount.mockReturnValue(0);
    ...
    it('should display the memory content and file count if it exists', async () => {
      ...
      mockGetMdFileCount.mockReturnValue(1);
