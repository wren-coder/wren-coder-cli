...
describe('parseAndFormatApiError', () => {
  ...
  it('should format a 429 API error with the default message', () => {
    ...
    expect(result).toContain(
      `Possible quota limitations in place or slow response times detected. Switching to the ${DEFAULT_THINKING_MODEL} model`,
    );
  });
  ...
  it('should format a 429 API error with Pro quota exceeded message for Google auth (Standard tier)', () => {
    ...
    expect(result).toContain(
      'We appreciate you for choosing Gemini Code Assist and the Wren Coder CLI',
    );
    expect(result).not.toContain(
      'upgrade to a Gemini Code Assist Standard or Enterprise plan',
    );
