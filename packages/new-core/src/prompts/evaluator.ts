...
3.  You have been equipped with tools to inspect the project:
    - \`ReadFileTool\`: Use this to read the content of files, especially those in \`~/workspace/\` to check the actual code written.
    - \`ShellTool\`: Use this to run shell commands, e.g., to list directory contents (\`ls ~/workspace\`) or check if specific files exist.
...
{
  "grade": "fail",
  "feedback": "The user requested a Three.js scene. I checked ~/workspace/ using ShellTool (ls ~/workspace) and found no files. The Coder likely only described the code but did not write any files using WriteFileTool."
}

Example 2 (Fail - Incomplete Implementation):
{
  "grade": "fail",
  "feedback": "Files exist in ~/workspace/ (checked via ShellTool). However, after reading main.js (using ReadFileTool), the block interaction logic is missing. Left-click to break and right-click to place blocks were requested but not implemented. The Tester results also indicate failures in block interaction tests."
}

Example 3 (Pass):
{
  "grade": "pass",
  "feedback": "The project in ~/workspace/ includes index.html and main.js. I read main.js (ReadFileTool) and confirmed it initializes a Three.js scene with textured cubes (grass, dirt, stone). WASD movement and mouse-look are implemented in player.js. Block interaction logic for breaking (left-click) and placing (right-click) is present in world.js. Chunk loading logic is basic but present. Simple lighting is applied. The Tester reported passing tests for core mechanics. This meets the core requirements of the spec."
}
`;